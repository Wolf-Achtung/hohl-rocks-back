/* hohl.rocks API – v5.1.0 (Gold, Ready) */
...
async function pingTavily(signal){
  if(!TAVILY_API_KEY) return 'skipped';
  try{
    const r = await fetch('https://api.tavily.com/search',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ api_key: TAVILY_API_KEY, query: 'ping', max_results: 1 }),
      signal
    });
    return r.ok ? 'ok' : 'http_'+r.status;
  }catch{ return 'err'; }
...
app.get('/readyz', async (_req,res)=>{
  try{
    ...
    res.status(500).json({ ok:false, error: e?.message || 'ready_failed' });
  }
});
...
app.use('/api', bubbleRouter); // includes GET /api/run (usage) + POST /api/run
