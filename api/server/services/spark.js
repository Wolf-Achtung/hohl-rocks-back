...
// Lightweight Service mit In-Memory-Cache und Provider-Stubs.
// Passt in eure CommonJS/Express-Struktur.

const crypto = require('crypto');

const rotation = (process.env.SPARK_ROTATION || 'prompt,insight,tool,funding')
...
// Kuratierte Defaults (laufen ohne Keys)
...
// Provider-Stubs (nutzen Keys, sonst null)
async function anthropicInsight(){
  if (!process.env.ANTHROPIC_API_KEY) return null;
  return {
    title: 'Quick Insight: DSGVO-Minimierung',
    teaser: '3 Datenfelder identifizieren, die Sie nicht mehr benötigen – plus Löschregel skizzieren.',
    url: '/tools/mini-audit'
  };
}
async function tavilyNews(){
  if (!process.env.TAVILY_API_KEY) return null;
  return {
    title: 'Aktuell: EU-AI-Act Fokus für KMU',
    teaser: 'Was kleine Unternehmen jetzt pragmatisch vorbereiten sollten (Checkliste).',
    url: '/news/eu-ai-act-kmu-checkliste'
  };
}
async function perplexityTool(){
  if (!process.env.PERPLEXITY_API_KEY) return null;
  return {
    title: 'Tool-Fund: EU-Hosting-Check',
    teaser: 'Schnell prüfen, ob ein KI-Tool EU-hosting & DPA anbietet.',
    url: '/tools/eu-hosting-check'
  };
}
...
async function buildItem(kind){
  try {
    if (kind === 'insight'){
      const r = await anthropicInsight(); if (!r) return null;
      return { date: todayYMD(), type: 'insight', title: r.title, teaser: r.teaser,
        cta: { label: 'Mehr lesen', url: r.url }, source: 'anthropic', expires_at: isoPlusDays(1),
        trace_id: 'spk_anthropic_' + Date.now() };
    }
    if (kind === 'tool'){
      const r = await perplexityTool(); if (!r) return null;
      return { date: todayYMD(), type: 'tool', title: r.title, teaser: r.teaser,
        cta: { label: 'Tool öffnen', url: r.url }, source: 'perplexity', expires_at: isoPlusDays(1),
        trace_id: 'spk_perplexity_' + Date.now() };
    }
    if (kind === 'funding' || kind === 'event'){
      const r = await tavilyNews(); if (!r) return null;
      return { date: todayYMD(), type: kind, title: r.title, teaser: r.teaser,
        cta: { label: 'Details', url: r.url }, source: 'tavily', expires_at: isoPlusDays(1),
        trace_id: 'spk_tavily_' + Date.now() };
    }
    if (kind === 'prompt'){
...
async function getTodaySpark(forceRefresh=false){
  const now = Date.now();
  if (!forceRefresh && cache.item && cache.expiresAt > now){
    return cache;
  }
